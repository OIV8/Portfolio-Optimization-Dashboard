...
# Sidebar Inputs
assets = st.sidebar.text_input("Enter ticker symbols (comma-separated)", "AAPL, MSFT, GOOG, AMZN")
bench = st.sidebar.text_input("Enter benchmark ticker symbol", "SPY")
start_date = st.sidebar.date_input("Start Date")
end_date = st.sidebar.date_input("End Date")
strategy = st.sidebar.selectbox("Optimization Strategy", ["Minimum Variance", "Maximum Sharpe", "Equal Weight", "Black-Litterman (Max Sharpe)"])
...
if st.sidebar.button("Optimize Portfolio"):
    ...
    benchmark = load_data(bench, start_date, end_date)

    if data is not None:
        opt_weights, stats = optimize_portfolio(data, strategy, benchmark, views=views)
